...
case class HtmlEmail(html: String, inlines: List[Annex[Attachable]], attachments: List[Annex[Attachable]])
...
object sendgridBackend extends SendmailBackend {
  ...
  def base64(data: Array[Byte]): String = new String(java.util.Base64.getEncoder().encode(data))
  ...
  def sendmail(to: Seq[String],
    ...
    fromEmail.setName(fromContact._1.replaceAll("\"", ""))
    fromEmail.setEmail(fromContact._2)
    mail.setFrom(fromEmail)

    mail.setSubject(subject)

    val personalization = new Personalization()
    to.map(extract).foreach { case (toName, toEmail) =>
      val to = new Email()
      to.setName(toName)
      to.setEmail(toEmail)
    ...
    cc.map(extract).foreach { case (ccName, ccEmail) =>
      val cc = new Email()
      cc.setName(ccName)
      cc.setEmail(ccEmail)
